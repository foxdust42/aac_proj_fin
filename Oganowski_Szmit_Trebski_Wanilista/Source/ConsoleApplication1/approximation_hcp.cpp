...
WeightedGraph transform_to_complete_weighted_graph(
  ...
  result.vertices = n;
  result.weightMatrix = std::vector<std::vector<int>>(
      n, std::vector<int>(n, 1));  // Initialize with weight 1
...
std::vector<int> nearest_neighbor(const std::vector<std::vector<int>>& weights,
                                  int start_vertex) {
  int n = weights.size();
...
std::vector<std::pair<int, int>> edges_to_add_in_cycle(const std::vector<int> cycle,
                          const std::vector<std::vector<int>>& weights, int n,
                          bool print = false, bool is_symmetric = false) {   
  ...
  for (int i = 0; i < cycle.size(); i++) {
    ...
    if (weights[from][to] == 1) {  // If edge doesn't exist in original graph
      edges_to_add.push_back(std::pair<int, int>(from, to));
    }
  }
...
bool three_opt_move(std::vector<int>& cycle,
        ...
        for (int f = 0; f < selectable_vertices.size(); f++) {
          ...
          for (int g = 0; g < second_selectable_vertices.size(); g++) {
            ...
            if (gain > best_gain) {
              ...
              //           << cycle[third_edge.start] << "->" <<
              //           cycle[third_edge.end]
              //           << std::endl;
  ...
  if (improved) {
    ...
    if (first_segment.start != first_segment.end) {
      ...
      std::vector<int> first_segment_vector(
          cycle.begin() + first_segment_min + 1,
    ...
    if (second_segment.start != second_segment.end) {
      ...
      std::vector<int> second_segment_vector(
          cycle.begin() + second_segment_min + 1,
          cycle.begin() + second_segment_max);

      if (second_segment.reverse) {
        std::reverse(second_segment_vector.begin(),
    ...
    if (third_segment.start > 0) {
      new_cycle.insert(new_cycle.end(), cycle.begin() + third_segment.start + 1,
                       cycle.end());
    }

    if (third_segment.end > 1) {
      new_cycle.insert(new_cycle.end(), cycle.begin() + 1,
                       cycle.begin() + first_edge.start);
    }
    ...
    // std::cout << "After improving: " << edges_to_add_in_cycle(new_cycle,
    // weights) << std::endl; print_cycle(new_cycle);
    cycle = new_cycle;
  }
...
std::vector<std::pair<int, int>> hamiltonian_completion_approximation(
  ...
  std::vector<int> cycle =
      nearest_neighbor(weighted_graph.weightMatrix, start_vertex);
  // std::cout << edges_to_add_in_cycle(cycle, weighted_graph.weightMatrix) <<
  // std::endl;
  bool improved;
  int iters = 0;
  ...
  return edges_to_add_in_cycle(cycle, weighted_graph.weightMatrix, n, true);
}